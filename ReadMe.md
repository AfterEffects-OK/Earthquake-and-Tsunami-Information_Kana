...
**最大震度3以上**の地震情報を一覧で確認できるほか、**緊急地震速報（予報）**にも対応しています。
...
*   **緊急地震速報（予報）の通知**: 緊急地震速報（予報）が発表された際に、タイトル横にアラートを表示し、通知音を再生して即座に通知します。通知音は聞き逃しを防ぐため2回再生されます。
...
*   **訓練モード**: 画面上のボタン一つで、津波警報を含む大規模な地震を想定した訓練用のダミーデータに切り替え、表示をテストすることができます。複数の大規模地震シナリオ（緊急地震速報を含む）が用意されています。

## 3. 画面構成と操作方法

**【重要】訓練モードについて**

このアプリケーションには訓練モードが搭載されていますが、訓練モードで表示される画面は実際の地震情報とは異なります。配信等で利用する際は、必ず通常モードで使用してください。訓練モードを有効にすると、複数のシナリオからランダムでEEWのテスト発報が行われます。


画面は大きく分けて「**メインエリア**（タイトル、緊急地震速報アラート、地震一覧、詳細パネル）」「**情報表示バー**（画面下部）」の2つのエリアで構成されています。
...
| **緊急地震速報の通知音** | 緊急地震速報（予報）受信時に通知音を再生するかどうかを設定します。音声は事前に読み込まれ、即座に2回再生されます。（デフォルト: ON） |
| **情報画面に表示する最低震度** | 情報表示バーで再生される震度情報の最低ラインを設定します。例えば「震度4以上」に設定すると、震度3の地域情報は再生されなくなります。（デフォルト: 震度3以上） |
| **地震一覧のフィルタ（最大震度）** | 左側の地震一覧パネルに表示する地震の最低ラインを設定します。例えば「最大震度4以上」に設定すると、最大震度3の地震は一覧に表示されなくなります。（デフォルト: 最大震度3以上） |
...
*   **設定の永続化**:
    *   「ショートカットキー」「情報画面に表示する最低震度」「地震一覧のフィルタ」「緊急地震速報の通知音」の設定は、ブラウザのローカルストレージに保存され、次回アクセス時も維持されます。
*   **音声のプリロード**:
    *   緊急地震速報の通知音は、アプリケーション読み込み時に事前にダウンロードされるため、受信時に即座に再生されます。

### 緊急地震速報（EEW）の発報基準について

このアプリケーションで受信する緊急地震速報（コード: 554）は、P2P地震情報APIから提供される「緊急地震速報 発表検出」の情報に基づいています。

#### P2P地震情報と気象庁のEEW

P2P地震情報は、主に気象庁が発表する防災情報を元にしています。気象庁が発表する緊急地震速報には、以下の2種類があります。

1.  **緊急地震速報（警報）**:
    *   **発表条件**: 震度5弱以上、または長周期地震動階級3以上の揺れが予想された場合。
    *   **対象**: 震度4以上の揺れが予想される地域に対して発表されます。

2.  **緊急地震速報（予報）**:
    *   **発表条件**: 震度3以上、またはマグニチュード3.5以上などの条件を満たした場合。
    *   **対象**: より広範囲に発表される速報です。

#### このアプリケーションでの動作

このアプリケーションが利用しているP2P地震情報APIの**コード`554`**は、気象庁が上記の「警報」または「予報」のいずれかを発表したという**事実を検知した時点**で通知されます。

そのため、震度3程度の揺れが予想される比較的軽微な速報から、震度5弱以上の強い揺れが予想される重大な速報まで、**気象庁がEEWを発表したすべてのケース**でアラートが表示される可能性があります。

> **補足**: P2P地震情報には、ユーザーが「揺れた」と感じた情報を共有する「地震感知情報」という独自の仕組みもありますが、このアプリケーションで利用しているコード`554`は、気象庁の公式発表をトリガーとする情報です。
